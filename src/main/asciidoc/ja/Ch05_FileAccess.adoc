...
レコード区分によって実行する処理を選択する場合の実装は、<<Ch05_FileAccess_MultiFormat>>を参考にすること。
====
...
``FieldExtractor``の実装例は<<Ch05_FileAccess_FixedLength_Output,固定長ファイルの出力>>にて、全角文字のフォーマットを例に説明しているためそちらを参照すること。
|icon:check[]
|なし
...
フィールドを囲み文字で囲む場合は、{batch5_shortname}が提供する``org.terasoluna.batch.item.file.transform.EnclosableDelimitedLineAggregator``を使用する。 +
...
====
{batch5_shortname}では、RFC-4180の仕様を満たすことを目的として拡張クラス``org.terasoluna.batch.item.file.transform.EnclosableDelimitedLineAggregator``を提供している。
...
レコードの区切りをバイト数で判断する場合は、{batch5_shortname}が提供する``org.terasoluna.batch.item.file.FixedByteLengthBufferedReaderFactory``を設定する。
...
|(6)
|lineTokenizer
|{batch5_shortname}が提供する``org.terasoluna.batch.item.file.transform.FixedByteLengthLineTokenizer``を設定する。
|icon:check[]
|なし
...
レコードの区切りをバイト数で判断するファイルを読み込む場合は、{batch5_shortname}が提供する``org.terasoluna.batch.item.file.FixedByteLengthBufferedReaderFactory``を使用する。
...
ヘッダ・フッタのレコード数が可変であり行数指定ができない場合は、<<Ch05_FileAccess_MultiLine_Input,マルチフォーマットの入力>>を参考に``PatternMatchingCompositeLineMapper``を使用すること。
...
{SB}および{batch5_shortname}では、フッタレコードの読み飛ばし機能は提供していないため、OSコマンドで対応する。
...
{SB}および{batch5_shortname}では、フッタレコードの読み飛ばし機能、フッタ情報の取得機能は提供していない。
...
** ``jobExecutionContext``を利用する方法は、JobとStepのスコープに関する違い以外は、<<Ch05_FileAccess_HeaderFooter_Input_AccessHeaders>>にて説明した``stepExecutionContext``と同様である。
...
``jobExecutionContext``からフッタ情報を取得し利用する方法は<<Ch05_FileAccess_HeaderFooter_Input_AccessHeaders>>と同じ要領で実現可能である。
...
フッタ情報を取得し利用する方法は<<Ch05_FileAccess_HeaderFooter_Input_AccessHeaders>>と同じ要領で実現可能である。
|
|
...
詳しい理由は、<<Ch05_FileAccess_HeaderFooter_Input_AccessHeaders>>を参照すること。
|
|なし
|===
...
** 実現方法は<<Ch05_FileAccess_HeaderFooter_Input_AccessHeaders>>と同様である
...
``Tasklet``の実装クラス内でBean定義した``ItemReader``を使用する方法は<<Ch03_CreateTaskletJob.adoc#Ch03_CreateTaskletJob_HowToUse_Implements,タスクレット指向ジョブの作成>>を参照すること。
...
|プロパティ``resource``は、``MultiResourceItemReader``から自動的に設定されるためBean定義に設定は不要である。
|icon:check[]
|
|===
...
====
``MultiResourceItemReader``から委譲される``ItemReader``の``resource``は、``MultiResourceItemReader``から自動的に設定されるためBean定義に設定は不要である。
====
...
|プロパティ``resource``は、``MultiResourceItemWriter``から自動的に設定されるためBean定義に設定は不要である。
|icon:check[]
|
|===
...
====
``MultiResourceItemWriter``から委譲される``ItemWriter``の``resource``は、``MultiResourceItemWriter``から自動的に設定されるためBean定義に設定は不要である。
====
...
コントロールブレイク処理をするためには、グループの変わり目を判定するために、レコードを先読みする必要がる。
...
* <<Ch05_FileAccess_Xml>>の入出力
* <<Ch05_FileAccess_MultiFormat>>の入出力
...
詳細は、<<Ch05_FileAccess_Xml_Output_format>>を参照すること。
====
...
変換対象クラスの定義および``ItemProcessor``定義例、注意点は<<Ch05_FileAccess_MultiLine_Input,マルチフォーマットの入力>>と同様である。
