...
==== {batch5_shortname}が提供する機能
{batch5_shortname}は、以下の機能を**非同期実行(DBポーリング)**として提供する。
...
==== ジョブ要求テーブルの構造
以下に、{batch5_shortname}が対応しているデータベース製品のうち、PostgreSQLの場合を示す。
その他のデータベースについては、{batch5_shortname}のjarに同梱されているDDLを参照してほしい。
...
[WARNING]
====
PostgreSQLはカラムの自動採番に対応しているため、{batch5_shortname}のjarに同梱されているDDLにジョブ要求シーケンスは定義されていない。
シーケンスの最大値を変更したい場合などに、``job_seq_id``のデータ型を``bigserial``から``bigint``に変更した上で、
ジョブ要求シーケンスを定義すると良い。
====
...
{batch5_shortname}のジョブ要求テーブルポーリング機能内部では、
...
ジョブステータス確認の詳細は、<<Ch07_JobManagement.adoc#Ch07_JobManagement_JobStatusManagement_Retrieve,"状態の確認">>を参照のこと。
...
停止方法は<<Ch07_JobManagement.adoc#Ch07_JobManagement_JobStatusManagement_JobStop,"ジョブの停止">>を参照のこと。
...
. ジョブ要求テーブルのカスタマイズ
. ``BatchJobRequestMapper``インターフェースの拡張インターフェースの作成
...
|(1)
|``BatchJobRequestMapper``の拡張インターフェースをFQCNでnamespaceに設定する。
...
|(1)
|``BatchJobRequestMapper``の拡張インターフェースをFQCNで``mapperInterface``プロパティに設定する。
...
[start=2]
. ``BatchJobRequestMapper``インターフェースの拡張インターフェース作成
...
(2)で作成した拡張インターフェースを``batchJobRequestMapper``に設定し、
...
=== ジョブ定義のモジュール化について
<<Ch04_AsyncJobWithDB_Arch_AppCtx>>でも簡単に説明したが、``AutomaticJobRegistrar``を用いることで以下の事象を回避することができる。
