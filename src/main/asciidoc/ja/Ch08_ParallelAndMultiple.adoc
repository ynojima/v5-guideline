...
ステップ単位で並列処理を行う。 +
各ステップは同じ処理である必要はなく、DBとファイルというような種類が異なるリソースに対して並列で処理を行う事も可能である。
...
非推奨理由::
  ...
  代わりに<<Ch08_ParallelAndMultiple_Partitioning>>を利用することを推奨する。
...
``next``属性による後続ステップの指定に関する詳細は<<Ch08_FlowControll.adoc#Ch08_FCh08_FlowControll_HowToUse_SequencialFlow,"シーケンシャルフロー">>を参照。
