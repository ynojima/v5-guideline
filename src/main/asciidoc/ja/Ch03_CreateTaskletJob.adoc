...
タスクレットモデルのアーキテクチャについては、<<Ch02_SpringBatchArchitecture.adoc#,{SB}のアーキテクチャ>>を参照のこと。
...
接頭辞として(8)で設定したid属性を付加した形式にする。
こうすることでステップもジョブと同様にバッチアプリケーションに含まれる全ジョブの範囲において一意となり、
...
また、``ref``属性に設定する値は、(3)により解決するBean名となる。 +
...
{batch5_shortname}では、以下のような状況に応じてこれを選択可能してよい。
...
|(9)
|データベースへ出力するため、``mybatis:scan``の設定を追加する。詳細はここでは割愛する。
...
<<Ch03_CreateTaskletJob_HowToUse_Implements_Simple>>の場合と同様となる。
