...
なお、アプリケーションのビルドや実行については、<<Ch03_CreateProject.adoc#Ch03_CreateProject,プロジェクトの作成>>を参照のこと。
また、起動パラメータの指定方法や活用方法については、<<Ch04_JobParameter.adoc#,ジョブの起動パラメータ>>を参照のこと。
...
-restart::
  失敗したジョブを再実行する。詳細は、<<Ch06_ReProcessing.adoc#,処理の再実行>>を参照のこと。
-stop::
  実行中のジョブを停止する。詳細は、<<Ch07_JobManagement.adoc#,ジョブの管理>>を参照のこと。
-abandon::
