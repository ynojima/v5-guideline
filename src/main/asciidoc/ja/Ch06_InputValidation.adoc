...
|エラーメッセージの設定
2+^|{server5_guide}の
{server5_url}/ArchitectureInDetail/WebApplicationDetail/Validation.html?highlight=validationmessages#validation-message-def[エラーメッセージの定義]と同様
...
[[Ch06_InputValidation_Overview_archtecture]]
=== 入力チェックの全体像
...
.入力チェックの関連クラス
image::Ch06_InputValidation_Architecture_classes.png[InputValidation archtecture]
...
{server5_guide}の {server5_url}/ArchitectureInDetail/WebApplicationDetail/Validation.html?highlight=validationmessages#validation-message-def[エラーメッセージの定義]
を参照すること。
...
** ``ItemProcessListener#onProcessError()``は``@OnProcessError``アノテーションを使用して実装してもよい。
   詳細は、<<Ch04_Listener.adoc#,リスナー>>を参照のこと。
...
終了コードの設定方法については、<<Ch07_JobManagement.adoc#,ジョブの管理>>を参照すること。
